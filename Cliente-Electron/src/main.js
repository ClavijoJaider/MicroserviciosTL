...
// BACKEND UNIFICADO - un solo Spring Boot en el puerto 8080.
// Mantenemos dos URLs base por simplicidad para no tocar el resto del codigo
// (ApiService se sigue instanciando dos veces) - ambas apuntan al mismo proceso.
const BACKEND_URL    = 'http://localhost:8080';
const CUENTAS_URL    = BACKEND_URL + '/cuentas';
const MOVIMIENTOS_URL = BACKEND_URL + '/movimientos';
...
/**
 * CuentaObserver: recibe notificaciones del backend unificado (SSE) y las
 * reenvia a todas las ventanas Electron abiertas mediante IPC.
 *
 * El Subject ahora es UN solo backend en el puerto 8080, pero expone dos
 * endpoints SSE (mismo stream interno):
 *   - /cuentas/eventos      -> CUENTA_CREADA, CUENTA_ACTUALIZADA, CUENTA_ELIMINADA
 *   - /movimientos/eventos  -> MOVIMIENTO_CREADO, MOVIMIENTO_ACTUALIZADO, MOVIMIENTO_ELIMINADO
 *
 * Nos suscribimos a ambos para preservar la separacion semantica del cliente.
 */
...
class ApiService extends IApiService {
  ...
  // POST /movimientos  — numeroCuenta va en el body; el backend unificado valida la cuenta in-process.
  ...
  const menu = Menu.buildFromTemplate([
    ...
    },
    {
      label: 'Movimientos', submenu: [
        ...
        { label: 'Buscar por ID',     click: () => vm.crear('buscar_movimiento_html',      { width: 480, height: 400, archivo: 'ventanas/buscar_movimiento_html.html',      parent: principal, preload: preloadPath }) },
      ...
      label: 'Ayuda', submenu: [
        {
          label: 'Acerca de...', click: () => {
            dialog.showMessageBox(principal, {
              ...
              message: 'WayBank - Sistema de Gestion Bancaria\n\nDesarrollado por:\n  Carlos Gil\n  Jaider Clavijo\n  Santiago Lozano\n\nVersion: 1.0.0\nArquitectura: REST + SSE + MVC\nBackend unificado: Spring Boot + Oracle (puerto 8080)',
              buttons: ['Cerrar']
            });
          }
        }
