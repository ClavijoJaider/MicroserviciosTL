 ...
 *
 * TERCER PROTOTIPO — relacion @ManyToOne:
 *   La relacion con CUENTA_AHORROS se declara con @ManyToOne en este lado (detalle).
 *   El campo {@code numeroCuenta} (int) sigue siendo la columna FK "propietaria" y
 *   es el campo que se usa en la serializacion JSON y en el codigo de servicio.
 *   El campo {@code cuenta} (@ManyToOne, insertable=false, updatable=false) es de
 *   solo lectura y permite navegar la relacion en consultas JPQL:
 *     FROM Movimiento m JOIN m.cuenta c WHERE ...
 *
 * SECUENCIA:
 *   SEQ_MOVIMIENTO — compatible con H2 (crea automaticamente) y Oracle XE.
 */
@Data
@Builder
@NoArgsConstructor
...
public class Movimiento {
    ...
    /**
     * Relacion @ManyToOne (TERCER PROTOTIPO).
     * insertable/updatable=false: la columna la gestiona {@code numeroCuenta}.
     * @JsonIgnore: evita serializacion circular; el titular se expone via DTO.
