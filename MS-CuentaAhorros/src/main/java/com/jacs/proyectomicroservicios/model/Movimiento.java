 ...
 *
 * BACKEND UNIFICADO — relacion @ManyToOne contra CUENTA_AHORROS:
 *   El campo {@code numeroCuenta} (int) sigue siendo la columna FK propietaria
 *   y es el atributo que se serializa en JSON.
 *   El campo {@code cuenta} (@ManyToOne, insertable=false, updatable=false) es
 *   de solo lectura y permite navegar la relacion en consultas JPQL:
 *     FROM Movimiento m JOIN m.cuenta c WHERE ...
 *
 *   Esto encaja con la nueva @OneToMany bidireccional en CuentaAhorros.
 *
 * SECUENCIA:
 *   SEQ_MOVIMIENTO — creada por Hibernate en Oracle al hacer ddl-auto=update.
 */
@Data
@Builder
@NoArgsConstructor
